...
<body style="margin-top:-35px;">
    ...
    @if(!empty($final_orders_3))
        <div class="page_break"></div>

        @php $items = []; @endphp
        <h1>Přehled pro {{$final_orders_3[0]['order']['date']}}</h1>

        <h3>Adresy:</h3>

        @foreach($final_orders_3 as $key => $order)
            @if(isset($order['order']))   
                <table>
                    <tr style="border-bottom:1px solid grey">
                        <td style="min-width:150px;">{{$key+1}}) {{$order['client']}}</td>
    ...
    @if(!empty($final_orders_4))
        <div class="page_break"></div>

        @php $items = []; @endphp
        <h1>Přehled pro {{$final_orders_4[0]['order']['date']}}</h1>

        <h3>Adresy:</h3>

        @foreach($final_orders_4 as $key => $order)
            @if(isset($order['order']))   
                <table>
                    <tr style="border-bottom:1px solid grey">
                        <td style="min-width:150px;">{{$key+1}}) {{$order['client']}}</td>
